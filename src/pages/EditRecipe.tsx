...
export default function EditRecipe() {
  ...
  // Fetch recipe data and dropdowns
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [recipeRes, categoriesRes, ingredientsRes] = await Promise.all([
        ...
        setCurrentImagePath(recipe.imagePath);
        ...
        setRecipeIngredients(
          recipe.ingredients.map((ing) => ({
            ...
            tempId: ing.ingredientId + Date.now(), // Unique temp ID
          }))
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedImage) {
        ...
        await api.post(`/recipe/${id}/image`, formData, {
  ...
  const currentImageUrl = currentImagePath
    ? `${import.meta.env.VITE_API_URL?.replace("/api", "")}${currentImagePath}`
    : null;
    ...
    >
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Ingredients - Same as CreateRecipe */}
          <div>
            ...
            {recipeIngredients.length === 0 ? (
              ...
              <div className="space-y-6">
                {" "}
                {/* ← Changed from space-y-3 to space-y-6 */}
                  ...
                  return (
                    <div key={ingredient.tempId} className="flex gap-3 items-end pb-2">
                      ...
                      <div className="w-32 relative">
                        ...
                        />
                        {isToTaste && (
                          <span className="absolute -bottom-5 left-0 text-xs text-darkTeal/60 italic whitespace-nowrap">
                            (optional)
                          </span>
                        )}
                      </div>
      ...
      {/* New Ingredient Modal - Same as CreateRecipe */}
